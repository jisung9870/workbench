...
var helpTopics = map[string]helpTopic{
	...
	"env":           {Path: "env", Summary: "Manage runtime environment metadata", Usage: "wb env <command> [options]", Commands: []helpCommand{{"list", "List Environments"}, {"show", "Show an Environment"}, {"add", "Create metadata with optional expiry"}, {"expiry", "Renew or clear expiry"}, {"remove", "Remove an Environment"}, {"health", "Check expiry and references"}, {"export", "Emit shell exports"}, {"migrate", "Migrate wenv.d presets"}}},
	"secrets":       {Path: "secrets", Summary: "Manage the encrypted local Secret store", Usage: "wb secrets <command> [options]", Commands: []helpCommand{{"init", "Initialize the store"}, {"list", "List names without values"}, {"set", "Store a value from stdin or hidden TTY"}, {"get", "Read a value to stdout"}, {"edit", "Edit a value in a protected temporary file"}, {"copy", "Copy a value with conditional auto-clear"}, {"remove", "Remove a service or field"}, {"migrate", "Migrate the legacy sec store"}}},
...
var leafUsage = map[string]string{
	...
	"env list": "wb env list [--json]", "env show": "wb env show <id> [--json]", "env add": "wb env add <id> [--ttl <duration>|--expires-at <RFC3339>] [metadata options] [--json]", "env expiry": "wb env expiry <id> (--ttl <duration>|--expires-at <RFC3339>|--clear) [--json]", "env remove": "wb env remove <id> [--json]", "env health": "wb env health <id> [--json]", "env export": "wb env export <id> [--resolve-secrets] [--json]", "env migrate": "wb env migrate check|apply [--source <wenv.d>] [--json]",
	"secrets init": "wb secrets init [--json]", "secrets list": "wb secrets list [service] [--json]", "secrets set": "wb secrets set <service> <field> [--replace] [--json]", "secrets get": "wb secrets get <service> [field]", "secrets edit": "wb secrets edit <service> <field> [--editor <executable>]", "secrets copy": "wb secrets copy <service> <field> [--clear-after <duration>]", "secrets remove": "wb secrets remove <service> [field] [--yes] [--json]", "secrets migrate": "wb secrets migrate check|apply [--json]",
