...
func runServerStart(args []string, paths config.Paths, stdout, stderr io.Writer) *commandError {
	...
	return &commandError{ExitCode: ExitPartial, Code: "SERVER_START_TIMEOUT", Message: fmt.Sprintf("Workbench server process %d started but did not become ready; inspect %s", state.PID, state.LogPath), Details: map[string]any{"pid": state.PID, "state_file": statePath, "log_file": state.LogPath}}
}
