...
func TestStopRejectsLegacyAndKillsOnlyMatchingManagedSession(t *testing.T) {
	...
	t.Run("managed", func(t *testing.T) {
		executor := newFakeExecutor()
		canonicalPath, err := projects.CanonicalPath(project.Path)
		if err != nil {
			t.Fatal(err)
		}
		executor.sessions[project.ID] = &fakeSession{
			windows: 1, start: project.Path,
			options: map[string]string{ManagedOption: "1", ProjectIDOption: project.ID, ProjectPathOption: canonicalPath},
		}
...
func TestJumpUsesCurrentClientOrAttachesOutsideTmux(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := manager.Attach(context.Background(), "alpha", true); err != nil {
				t.Fatal(err)
			}
...
func TestAttachRejectsNonInteractiveDashboardOutsideTmux(t *testing.T) {
	executor := newFakeExecutor()
	executor.sessions["alpha"] = &fakeSession{windows: 1, start: "/tmp", options: map[string]string{}}
	_, err := NewManager(executor, func(string) string { return "" }).Attach(context.Background(), "alpha", false)
	var conflict *ConflictError
	if !errors.As(err, &conflict) {
		t.Fatalf("non-interactive attach was not rejected: %v", err)
	}
	for _, call := range executor.calls {
		if len(call.Args) > 0 && call.Args[0] == "attach-session" {
			t.Fatalf("dashboard started an interactive attachment: %#v", call)
		}
	}
}
